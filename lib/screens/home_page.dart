// ignore_for_file: deprecated_member_use, avoid_print, strict_top_level_inference
...
class _HomePageState extends State<HomePage> {
  ...
  Future<Object?>? getWeatherData() async {
    try {
      final response = await http.get(
        Uri.parse(
          OpenWeatherAPI().apiUrl(lat, lon),
        ),
      );
  ...
  void getHourlyData() {
    for (int i = 0; i < weatherData!.getHourlyWeather().hourly.length; i++) {
      ...
      hourlyIcon.add(
        weatherData!.getHourlyWeather().hourly[i].weather![0].icon.toString(),
      );
    }
  }
